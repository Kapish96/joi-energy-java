...
public class PricePlanService {
	...
	public BigDecimal getLastWeekMeterUsageCost(String smartMeterId, String plan) {
		
		Map<String, PricePlan> pricePlanMap = pricePlans.stream().collect(Collectors.toMap(PricePlan::getPlanName, Function.identity()));
		
		List<ElectricityReading> electricityReadings = meterReadingService.getReadings(smartMeterId).get();
		
		 electricityReadings =  electricityReadings.stream()
				.filter(e -> e.getTime()
						.compareTo(LocalDate.now().minusDays(7).atStartOfDay(ZoneId.systemDefault()).toInstant())>=0)
				.collect(Collectors.toList());
		 BigDecimal cost = calculateCost(electricityReadings, pricePlanMap.get(plan));
		return cost;
	}
